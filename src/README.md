...
- `renderer.ts`：公开状态组件、模型 provider 标识、session、token/cache/cost 展示、可复用 `QuotaWindow`/`QuotaGroup` formatter、独立 provider quota segment、逻辑行布局、Powerline 渲染和物理宽度约束。
